...
function mostrarAsistentesActo(idActo) {
  const xhttp = new XMLHttpRequest();
  xhttp.addEventListener("readystatechange", function () {
    if (this.readyState == 4 && this.status == 200) {
      dibujarDetalleAsistentesActo(JSON.parse(this.responseText));
    } else if (this.status == 403) {
      window.location.href = "../login.html";
    }
  });

  xhttp.open("GET", "../src/asistentesEvento.php?id=" + idActo, true);
  xhttp.send();
}


function dibujarDetalleAsistentesActo(datos) {
 
  bodyinscritos = document.getElementById("bodyInscritos");

  bodyinscritos.innerHTML = "";
  ...
  for (valor in datos) {
    let fila = document.createElement("tr");
    let campo = document.createElement("td");
    campo.innerHTML = datos[valor]["id"] + " "+datos[valor]["nombre"] + " " +datos[valor]["apellidos"];
    fila.appendChild(campo);
    bodyinscritos.appendChild(fila);
